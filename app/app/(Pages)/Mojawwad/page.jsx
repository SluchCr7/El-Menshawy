...
import { Mic2, Star, Search, Sparkles } from 'lucide-react';
...
const MojawwadQuranPage = () => {
  const [searchQuery, setSearchQuery] = useState('');

  const filteredSurahs = menshQuran.filter(surah => {
    ...
    <main className="min-h-screen bg-cream">
      ...
      <section className="relative pt-40 pb-24 overflow-hidden bg-primary-dark">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          >
            <Mic2 size={40} className="text-accent" />
          </motion.div>
          <motion.h1
          ...
          >
            روائع التلاوة المجودة التي اشتهر بها "الصوت الباكي"، حيث يجتمع الإتقان الفني مع الخشوع العميق في أروع صور الأداء القرآني.
          </motion.p>
        </div>
      </section>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-8 bg-white/80 backdrop-blur-md p-6 rounded-[2rem] shadow-xl border border-accent/10">
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                <p className="text-xs font-sans uppercase tracking-[0.2em] text-primary/40">Pure Heritage</p>
                <p className="text-xl font-reem font-bold text-primary">
                  {filteredSurahs.length === menshQuran.length
                    ? `${menshQuran.length} سورة مسجلة`
            ...
            <div className="relative w-full md:w-96">
              <input
                type="text"
                placeholder="ابحث عن السورة باسمها أو رقمها..."
                value={searchQuery}
...
export default MojawwadQuranPage;